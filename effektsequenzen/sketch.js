...
function draw() {
  ...
  if (kinectImage) {
    ...
    if (wasPersonPresent && !personPresentNow) {
      // Nur speichern und spawnen, wenn Bewegung lang genug war (mind. 5 Frames)
...
function hasPerson(img) {
  ...
  currentPixelMass = darkPixelCount; // Wert an globale Variable für UI übergeben
...
function keyPressed(){
  if (key === 'b' || key === 'B'){
    ...
    console.log("Background saved command gesendet");
  }
